<template>
  <tr
    class="gamelist-item"
    v-if="title === 'past' && tab === 'created'"
    @click="sendTo('rank', props.aMatch.id)"
  >
  ...
  <tr
    class="gamelist-item"
    v-if="title === 'past' && tab === 'joined'"
    @click="sendTo('rank', props.aMatch.id)"
  >
    <td>{{ props.aMatch.acf.title }}<br />{{ formatDate(props.aMatch.acf.end_date) }}</td>
    <td>{{ rankingFirst.name }}<br />{{ ranking[0].time }} s</td>
    <td>
      {{ myRank + 1 }}/{{ theMatchPlayers.length }}<br /><span v-if="theMatch && myTime.length > 0"
        >{{ myTime[0].time }}s</span
      >
    </td>
  </tr>
  <tr class="gamelist-item" v-if="title === 'present'" @click="sendTo('game', props.aMatch.id)">
    <td>
      #{{ props.aMatch.id }} - {{ props.aMatch.acf.title }}<br />{{
        props.aMatch.acf.players.length
      }}
      joueurs
    </td>
